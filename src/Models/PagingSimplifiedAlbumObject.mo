...
module {
    /// The required-fields slice of PagingSimplifiedAlbumObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express PagingSimplifiedAlbumObject as an
    ...
    public type PagingSimplifiedAlbumObject = Required and Optional;

    public module JSON {
        // `init` constructs a PagingSimplifiedAlbumObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { PagingSimplifiedAlbumObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : PagingSimplifiedAlbumObject {
            let ?res = from_candid(to_candid(required)) : ?PagingSimplifiedAlbumObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : PagingSimplifiedAlbumObject) : Candid.Candid {
            ...
            List.add(buf, ("items", #Array(Array.map<SimplifiedAlbumObject, Candid.Candid>(value.items, SimplifiedAlbumObject.toCandidValue))));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?PagingSimplifiedAlbumObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let ?items = ((switch (items_field.1) {
                        case (#Array(xs__)) {
                            let buf__ = List.empty<SimplifiedAlbumObject>();
                            for (c__ in xs__.values()) {
                                let ?m__ = SimplifiedAlbumObject.fromCandidValue(c__) else return null;
                                List.add(buf__, m__);
                            };
                            ?List.toArray(buf__);
                        };
