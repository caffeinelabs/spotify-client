...
module {
    /// The required-fields slice of SimplifiedAlbumObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express SimplifiedAlbumObject as an
    // `and`-intersection and keep `init` from listing every optional explicitly.
    type Optional = {
        restrictions : ?AlbumRestrictionObject;
    };

    public type SimplifiedAlbumObject = Required and Optional;

    public module JSON {
        // `init` constructs a SimplifiedAlbumObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { SimplifiedAlbumObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : SimplifiedAlbumObject {
            let ?res = from_candid(to_candid(required)) : ?SimplifiedAlbumObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : SimplifiedAlbumObject) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?SimplifiedAlbumObject =
            switch (candid) {
                case (#Record(fields)) {
