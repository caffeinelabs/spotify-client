...
module {
    /// The required-fields slice of SimplifiedPlaylistObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express SimplifiedPlaylistObject as an
    ...
    public type SimplifiedPlaylistObject = Required and Optional;

    public module JSON {
        // `init` constructs a SimplifiedPlaylistObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { SimplifiedPlaylistObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : SimplifiedPlaylistObject {
            let ?res = from_candid(to_candid(required)) : ?SimplifiedPlaylistObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : SimplifiedPlaylistObject) : Candid.Candid {
            ...
            switch (value.tracks) {
                case (?v__) List.add(buf, ("tracks", PlaylistTracksRefObject.toCandidValue(v__)));
                case null ();
            };
            switch (value.type_) {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?SimplifiedPlaylistObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let tracks : ?PlaylistTracksRefObject = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "tracks")) {
                        case (?tracks_field) (PlaylistTracksRefObject.fromCandidValue(tracks_field.1));
                        case null null;
                    };
