...
module {
    /// The required-fields slice of ShowObject — what `init` consumes.
    ...
    public type Required = {
        ...
        episodes : Candid.Candid;
    };

    // Optional-fields slice. Private — not part of the consumer surface;
    // it's an internal scaffold so we can express ShowObject as an
    // `and`-intersection and keep `init` from listing every optional explicitly.
    type Optional = {
    };

    public type ShowObject = Required and Optional;

    public module JSON {
        // `init` constructs a ShowObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { ShowObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : ShowObject {
            let ?res = from_candid(to_candid(required)) : ?ShowObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : ShowObject) : Candid.Candid {
            ...
            List.add(buf, ("episodes", value.episodes));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?ShowObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let ?episodes = (?episodes_field.1) else return null;
                    ?{
                        available_markets;
                        copyrights;
                        description;
