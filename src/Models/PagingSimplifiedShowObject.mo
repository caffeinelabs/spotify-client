...
module {
    /// The required-fields slice of PagingSimplifiedShowObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express PagingSimplifiedShowObject as an
    ...
    public type PagingSimplifiedShowObject = Required and Optional;

    public module JSON {
        // `init` constructs a PagingSimplifiedShowObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { PagingSimplifiedShowObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : PagingSimplifiedShowObject {
            let ?res = from_candid(to_candid(required)) : ?PagingSimplifiedShowObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : PagingSimplifiedShowObject) : Candid.Candid {
            ...
            List.add(buf, ("items", #Array(Array.map<SimplifiedShowObject, Candid.Candid>(value.items, SimplifiedShowObject.toCandidValue))));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?PagingSimplifiedShowObject =
            switch (candid) {
                case (#Record(fields)) {
