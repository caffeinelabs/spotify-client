...
module {
    /// The required-fields slice of PagingTrackObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express PagingTrackObject as an
    ...
    public type PagingTrackObject = Required and Optional;

    public module JSON {
        // `init` constructs a PagingTrackObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { PagingTrackObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : PagingTrackObject {
            let ?res = from_candid(to_candid(required)) : ?PagingTrackObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : PagingTrackObject) : Candid.Candid {
            ...
            List.add(buf, ("items", #Array(Array.map<TrackObject, Candid.Candid>(value.items, TrackObject.toCandidValue))));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?PagingTrackObject =
            switch (candid) {
                case (#Record(fields)) {
