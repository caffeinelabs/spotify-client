...
module {
    /// The required-fields slice of PagingSimplifiedAudiobookObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express PagingSimplifiedAudiobookObject as an
    ...
    public type PagingSimplifiedAudiobookObject = Required and Optional;

    public module JSON {
        // `init` constructs a PagingSimplifiedAudiobookObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { PagingSimplifiedAudiobookObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : PagingSimplifiedAudiobookObject {
            let ?res = from_candid(to_candid(required)) : ?PagingSimplifiedAudiobookObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : PagingSimplifiedAudiobookObject) : Candid.Candid {
            ...
            List.add(buf, ("items", #Array(Array.map<SimplifiedAudiobookObject, Candid.Candid>(value.items, SimplifiedAudiobookObject.toCandidValue))));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?PagingSimplifiedAudiobookObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let ?items = ((switch (items_field.1) {
                        case (#Array(xs__)) {
                            let buf__ = List.empty<SimplifiedAudiobookObject>();
                            for (c__ in xs__.values()) {
                                let ?m__ = SimplifiedAudiobookObject.fromCandidValue(c__) else return null;
                                List.add(buf__, m__);
                            };
                            ?List.toArray(buf__);
                        };
