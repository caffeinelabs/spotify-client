...
module {
    /// The required-fields slice of SimplifiedTrackObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express SimplifiedTrackObject as an
    ...
    public type SimplifiedTrackObject = Required and Optional;

    public module JSON {
        // `init` constructs a SimplifiedTrackObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { SimplifiedTrackObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : SimplifiedTrackObject {
            let ?res = from_candid(to_candid(required)) : ?SimplifiedTrackObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : SimplifiedTrackObject) : Candid.Candid {
            ...
            switch (value.linked_from) {
                case (?v__) List.add(buf, ("linked_from", LinkedTrackObject.toCandidValue(v__)));
                case null ();
            };
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?SimplifiedTrackObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let linked_from : ?LinkedTrackObject = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "linked_from")) {
                        case (?linked_from_field) (LinkedTrackObject.fromCandidValue(linked_from_field.1));
                        case null null;
                    };
