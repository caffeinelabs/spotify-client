...
module {
    /// The required-fields slice of ChapterObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express ChapterObject as an
    ...
    public type ChapterObject = Required and Optional;

    public module JSON {
        // `init` constructs a ChapterObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { ChapterObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : ChapterObject {
            let ?res = from_candid(to_candid(required)) : ?ChapterObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : ChapterObject) : Candid.Candid {
            ...
            List.add(buf, ("chapter_number", #Int(value.chapter_number)));
            ...
            switch (value.restrictions) {
                case (?v__) List.add(buf, ("restrictions", ChapterRestrictionObject.toCandidValue(v__)));
                case null ();
            };
            List.add(buf, ("audiobook", SimplifiedAudiobookObject.toCandidValue(value.audiobook)));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?ChapterObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let ?chapter_number_field = Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "chapter_number") else return null;
                    let ?chapter_number = ((switch (chapter_number_field.1) { case (#Int(i)) ?i; case (#Nat(n)) ?n; case _ null })) else return null;
                    ...
                    let restrictions : ?ChapterRestrictionObject = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "restrictions")) {
                        case (?restrictions_field) (ChapterRestrictionObject.fromCandidValue(restrictions_field.1));
                        case null null;
                    };
                    let ?audiobook_field = Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "audiobook") else return null;
                    let ?audiobook = (SimplifiedAudiobookObject.fromCandidValue(audiobook_field.1)) else return null;
                    ?{
                        audio_preview_url;
                        available_markets;
                        chapter_number;
