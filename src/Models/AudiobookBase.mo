...
module {
    /// The required-fields slice of AudiobookBase — what `init` consumes.
    ...
    // it's an internal scaffold so we can express AudiobookBase as an
    // `and`-intersection and keep `init` from listing every optional explicitly.
    type Optional = {
        edition : ?Text;
    };

    public type AudiobookBase = Required and Optional;

    public module JSON {
        // `init` constructs a AudiobookBase from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { AudiobookBase.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : AudiobookBase {
            let ?res = from_candid(to_candid(required)) : ?AudiobookBase else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : AudiobookBase) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?AudiobookBase =
            switch (candid) {
                case (#Record(fields)) {
