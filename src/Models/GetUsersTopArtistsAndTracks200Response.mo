...
module {
    /// The required-fields slice of GetUsersTopArtistsAndTracks200Response — what `init` consumes.
    ...
    // it's an internal scaffold so we can express GetUsersTopArtistsAndTracks200Response as an
    ...
    public type GetUsersTopArtistsAndTracks200Response = Required and Optional;

    public module JSON {
        // `init` constructs a GetUsersTopArtistsAndTracks200Response from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { GetUsersTopArtistsAndTracks200Response.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : GetUsersTopArtistsAndTracks200Response {
            let ?res = from_candid(to_candid(required)) : ?GetUsersTopArtistsAndTracks200Response else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : GetUsersTopArtistsAndTracks200Response) : Candid.Candid {
            ...
            List.add(buf, ("items", #Array(Array.map<GetUsersTopArtistsAndTracks200ResponseAllOfItemsInner, Candid.Candid>(value.items, GetUsersTopArtistsAndTracks200ResponseAllOfItemsInner.toCandidValue))));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?GetUsersTopArtistsAndTracks200Response =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let ?items = ((switch (items_field.1) {
                        case (#Array(xs__)) {
                            let buf__ = List.empty<GetUsersTopArtistsAndTracks200ResponseAllOfItemsInner>();
                            for (c__ in xs__.values()) {
                                let ?m__ = GetUsersTopArtistsAndTracks200ResponseAllOfItemsInner.fromCandidValue(c__) else return null;
                                List.add(buf__, m__);
                            };
                            ?List.toArray(buf__);
                        };
