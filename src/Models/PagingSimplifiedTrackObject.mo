...
module {
    /// The required-fields slice of PagingSimplifiedTrackObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express PagingSimplifiedTrackObject as an
    ...
    public type PagingSimplifiedTrackObject = Required and Optional;

    public module JSON {
        // `init` constructs a PagingSimplifiedTrackObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { PagingSimplifiedTrackObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : PagingSimplifiedTrackObject {
            let ?res = from_candid(to_candid(required)) : ?PagingSimplifiedTrackObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : PagingSimplifiedTrackObject) : Candid.Candid {
            ...
            List.add(buf, ("items", #Array(Array.map<SimplifiedTrackObject, Candid.Candid>(value.items, SimplifiedTrackObject.toCandidValue))));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?PagingSimplifiedTrackObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let ?items = ((switch (items_field.1) {
                        case (#Array(xs__)) {
                            let buf__ = List.empty<SimplifiedTrackObject>();
                            for (c__ in xs__.values()) {
                                let ?m__ = SimplifiedTrackObject.fromCandidValue(c__) else return null;
                                List.add(buf__, m__);
                            };
                            ?List.toArray(buf__);
                        };
