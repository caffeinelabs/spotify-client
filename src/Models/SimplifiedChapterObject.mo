...
module {
    /// The required-fields slice of SimplifiedChapterObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express SimplifiedChapterObject as an
    ...
    public type SimplifiedChapterObject = Required and Optional;

    public module JSON {
        // `init` constructs a SimplifiedChapterObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { SimplifiedChapterObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : SimplifiedChapterObject {
            let ?res = from_candid(to_candid(required)) : ?SimplifiedChapterObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : SimplifiedChapterObject) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?SimplifiedChapterObject =
            switch (candid) {
                case (#Record(fields)) {
