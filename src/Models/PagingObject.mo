...
module {
    /// The required-fields slice of PagingObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express PagingObject as an
    ...
    public type PagingObject = Required and Optional;

    public module JSON {
        // `init` constructs a PagingObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { PagingObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : PagingObject {
            let ?res = from_candid(to_candid(required)) : ?PagingObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : PagingObject) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?PagingObject =
            switch (candid) {
                case (#Record(fields)) {
