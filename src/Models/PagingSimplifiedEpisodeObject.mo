...
module {
    /// The required-fields slice of PagingSimplifiedEpisodeObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express PagingSimplifiedEpisodeObject as an
    ...
    public type PagingSimplifiedEpisodeObject = Required and Optional;

    public module JSON {
        // `init` constructs a PagingSimplifiedEpisodeObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { PagingSimplifiedEpisodeObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : PagingSimplifiedEpisodeObject {
            let ?res = from_candid(to_candid(required)) : ?PagingSimplifiedEpisodeObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : PagingSimplifiedEpisodeObject) : Candid.Candid {
            ...
            List.add(buf, ("items", #Array(Array.map<SimplifiedEpisodeObject, Candid.Candid>(value.items, SimplifiedEpisodeObject.toCandidValue))));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?PagingSimplifiedEpisodeObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let ?items = ((switch (items_field.1) {
                        case (#Array(xs__)) {
                            let buf__ = List.empty<SimplifiedEpisodeObject>();
                            for (c__ in xs__.values()) {
                                let ?m__ = SimplifiedEpisodeObject.fromCandidValue(c__) else return null;
                                List.add(buf__, m__);
                            };
                            ?List.toArray(buf__);
                        };
