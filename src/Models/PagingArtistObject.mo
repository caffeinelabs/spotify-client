...
module {
    /// The required-fields slice of PagingArtistObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express PagingArtistObject as an
    ...
    public type PagingArtistObject = Required and Optional;

    public module JSON {
        // `init` constructs a PagingArtistObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { PagingArtistObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : PagingArtistObject {
            let ?res = from_candid(to_candid(required)) : ?PagingArtistObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : PagingArtistObject) : Candid.Candid {
            ...
            List.add(buf, ("items", #Array(Array.map<ArtistObject, Candid.Candid>(value.items, ArtistObject.toCandidValue))));
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?PagingArtistObject =
            switch (candid) {
                case (#Record(fields)) {
