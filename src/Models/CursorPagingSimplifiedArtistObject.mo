...
module {
    /// The required-fields slice of CursorPagingSimplifiedArtistObject — what `init` consumes.
    ...
    // it's an internal scaffold so we can express CursorPagingSimplifiedArtistObject as an
    ...
    public type CursorPagingSimplifiedArtistObject = Required and Optional;

    public module JSON {
        // `init` constructs a CursorPagingSimplifiedArtistObject from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { CursorPagingSimplifiedArtistObject.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : CursorPagingSimplifiedArtistObject {
            let ?res = from_candid(to_candid(required)) : ?CursorPagingSimplifiedArtistObject else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : CursorPagingSimplifiedArtistObject) : Candid.Candid {
            ...
            switch (value.items) {
                case (?v__) List.add(buf, ("items", #Array(Array.map<ArtistObject, Candid.Candid>(v__, ArtistObject.toCandidValue))));
                case null ();
            };
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?CursorPagingSimplifiedArtistObject =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let items : ?[ArtistObject] = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "items")) {
